...
class Khaleesi: public IterativeRobot
{
	...
	enum moatEnum
	{
		moatIntake, moatFirstBump, moatBack, moatSecondBump, moatDrive, moatShoot
	} moatState;

	enum rockWallEnum
	{
		rockWallIntake, rockWallApproach, rockWallCross, rockWallDrive, rockWallShoot
	} rockWallState;
	...
	enum chevalEnum
	{
		chevalIntake, chevalApproach, chevalPutDownArm, chevalCross, chevalSetUpShot, chevalShoot
	} chevalState;
	...
	{//TODO: I need it
		...
		case 1:
			//Drive PID for pos1
			encCur = (left->GetEncPosition() + left2->GetEncPosition() + right->GetEncPosition() + right2->GetEncPosition()/4);
			encTar = 217;
			setSpeed(normPID(encTar, encCur, 217, 217));
			break;
		case 2:
			//Drive PID for pos2
			encCur = (left->GetEncPosition() + left2->GetEncPosition() + right->GetEncPosition() + right2->GetEncPosition()/4);
		...
		case chevalIntake:
			...
			if (ballHit == 1) {
				setIntake(0);
				chevalState = chevalApproach;
				vertGyro->Reset();
			}
			break;

		case chevalApproach:
			//Drive PID to bring the bot into position on the cheval
			chevalState = chevalPutDownArm;
			break;


		case chevalPutDownArm:
			//Arm PID to push down the cheval
			chevalState = chevalCross;
			break;

		case chevalCross:
			//drive PID to cross the cheval
			chevalState = chevalSetUpShot;
			break;

		case chevalSetUpShot:
			//Do what we need to to make sure the robot is in a position to accurately
			//do Vision to make a shot
			chevalState = chevalShoot;
			break;
	...
	void Moat(int pos)
	{
		...
		switch (moatState)
		{
			...
			case moatFirstBump:
				...
				if (vertGyro->GetRate() >= 25 && (antiTipEngaged == false))
				{
					left->Set(0);
					right->Set(0);
					Wait(.5);
			...
			case moatSecondBump:
				...
				if (vertGyro->GetRate() < -89 && (antiTipEngaged == false))
				{
					left->Set(0);
					right->Set(0);
					moatState = moatDrive;
	...
	void RockWall(int pos)
	{
		...
		switch (rockWallState)
		{
			...
			case rockWallApproach:
				...
				if (vertGyro->GetRate() >= 60 && (antiTipEngaged == false))
				{
					Wait(.1);
					vertGyro->GetRate();
			...
			case rockWallCross:
				...
				//armPID(0);
				if (vertGyro->GetRate() <= -90 && (antiTipEngaged == false))
				{
					left->Set(0);
					right->Set(0);
					Wait(1);
	...
	void Ramparts(int pos)
	{
		...
		switch (rampartsState)
		{
			...
			case rampartsApproach:
				...
				if (vertGyro->GetRate() <= -43 && (antiTipEngaged == false))
				{
					Wait(.1);
					vertGyro->GetRate();
			...
			case rampartsCross:
				...
				if (vertGyro->GetRate() <= -43 && rampartsFlag && (antiTipEngaged == false))
				{
					vertGyro->GetRate();
					left->Set(0);
					right->Set(0);
			...
			case rampartsRotate:
				...
				{
					gyroTar=0;
					left->Set(gyroPID(gyroTar));
					right->Set(gyroPID(gyroTar));
					antiTipEngaged=false;
				}
				if (((absVal(horzGyro->GetAngle()) < 5 ) || (rampartsTimer->Get() > 2)) && (antiTipEngaged == false))
				{
					left->Set(0);
					right->Set(0);
					rampartsState=rampartsShoot;
	...
	void TeleopPeriodic()
	{
		...
		{
//			if (vertGyro->GetAngle() < antiTipAngle && ptoEngaged == true)//pto is not engaged
//			{
//				left->Set(-.65);
//				right->Set(.65);
//			}
...
//						"Shooting" : "Standby");
	...
	void Hood()
	{
		...
		else
		{
			...
			{
				hood->Set(normPID(980, hood->GetEncPosition(), .0048, 0));
				flyWheelTarget = 3300;
			}
			if (oper->GetPOV() == 0)
			{
				hood->Set(normPID(840, hood->GetEncPosition(), .0048, 0));
				flyWheelTarget = 3265;
			}
			if (oper->GetPOV() == 90)
			{
				hood->Set(normPID(610, hood->GetEncPosition(), .0048, 0), 0);//590 in quals
				flyWheelTarget = 3600;
			}

			if (oper->GetPOV() == 180)
			{
				hood->Set(normPID(660, hood->GetEncPosition(), .0048, 0), 0);//690
				flyWheelTarget = 3600;
			}

			if (oper->GetPOV() == 270)
			{
				hood->Set(normPID(855, hood->GetEncPosition(), .0048, 0), 0);//1000
				flyWheelTarget = 3600;
			}
		}
	}
...
//		SmartDashboard::PutString("DB/String 2", climbString);
	...
	void visionYAlign()
	{
		...
		{
			hood->Set(normPID(600, hood->GetEncPosition(), .0048, 0));
		}
		else if (cogy < 55)
		{
			hood->Set(normPID(600, hood->GetEncPosition(), .0048, 0));
		...
		{
			hood->Set(normPID(580, hood->GetEncPosition(), .0048, 0));
		}
		else if (cogy < 110)
		{
			hood->Set(normPID(600, hood->GetEncPosition(), .0048, 0));
		...
		{
			hood->Set(normPID(730, hood->GetEncPosition(), .0048, 0));
		}
		else if (cogy < 165)
		{
			hood->Set(normPID(810, hood->GetEncPosition(), .0048, 0));
		}
		else
			hood->Set(normPID(865, hood->GetEncPosition(), .0048, 0));
