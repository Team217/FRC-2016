...
public class Robot extends IterativeRobot {
	...
	public void teleopPeriodic() {
		...
		if (driver.getRawButton(7)) {
			...
			if (vertGyro.getAngle() < antiTipAngle && ptoEngaged == true && !gyroDisable) // pto
			// engaged
			{
				left.set(-.65);
				right.set(.65);
	...
	void Moat(int pos) {
		...
		case moatRotate:
			...
			if (Math.abs(horzGyro.getAngle() - angleTar) < 5) {
				left.set(0);
				right.set(0);
	...
	boolean secondPos() {
		...
		case turn1:
			left.set(.5);
			right.set(.5);
			if (Math.abs(horzGyro.getAngle() - 45) < 5.0) {
		...
		case turn2:
			left.set(-.33);
			right.set(-.33);
			if (Math.abs(horzGyro.getAngle()) < 5.0)
				return true;
			else
				return false;
	...
	void SmartDash() {
		...
		// std.Value.to_string(hforzGyro.getAngle()));
		...
		String gyroString = "Shooter: " + String.valueOf(flyWheel.getSpeed());
		SmartDashboard.putString("DB/String 2", gyroString);

		// SmartDashboard.Value.PutString("DB/String 2",
		// (Math.abs(flyWheelSpeed) > 40
