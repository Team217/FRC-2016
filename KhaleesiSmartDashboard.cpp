...
private:
	...
	const string nothing = "Do Nothing";
	string defenseSelected;

	//Position Option Select
	SendableChooser *positionChooser;
	const string firstPosition = "Position 1";
	const string secondPosition = "Position 2";
	const string thirdPosition = "Position 3";
	const string fourthPosition = "Position 4";
	const string fifthPosition = "Position 5";
	...
	DigitalInput *bannerInner, *zeroHood, *climberSwitch;
	bool ballHit = false;
	...
	void RobotInit() {
		...
		defenseChooser->AddDefault(nothing, (void*) &nothing);
		...
		positionChooser->AddObject(fifthPosition, (void*) &fifthPosition);
	...
	void SmartDash() {
		...
		SmartDashboard::PutBoolean("Shooter Running",
				absVal(shooter1->GetSpeed()) >= 1000);

		SmartDashboard::PutBoolean("Ball in place", ballHit);
		SmartDashboard::PutBoolean("PTO", ptoEngaged);
		SmartDashboard::PutBoolean("Climber Angle", (bool) climberAngle->Get());
		SmartDashboard::PutBoolean("Climb Release", (bool) climbLock->Get());

		SmartDashboard::PutNumber("Hood Position", hood->GetEncPosition());
		SmartDashboard::PutNumber("Arm position", arm->GetEncPosition());

		SmartDashboard::PutNumber("X Angle", horzGyro->GetAngle());
		SmartDashboard::PutNumber("Y Angle", vertGyro->GetAngle());
		//SmartDashboard::PutNumber("Y Angle Rate", abs(vertGyro->GetRate()) >= 1.0 ? 0 : vertGyro->GetRate());
	}
...
//Sets speed of intake motors
	...
	void positionChoosing() {
		...
		if (positionSelected == fourthPosition)
			position = 4;
		if (positionSelected == fifthPosition)
			position = 5;
	}
