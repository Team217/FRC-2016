...
class Khaleesi: public IterativeRobot
{
	...
	//Shooter PID
	float shooterError, shooterTarget, shooterSpeed, shooterAvg, pOut, iOut, output;
	...
	bool ptoEngaged = false;bool angleSet = false;bool pidArm = false;bool angleReleased = true;
	...
	enum spyEnum
	{
		spyIntake, spyFirstTurn, spyDrive, spySecondTurn, spyShoot
	} spyState;
...
//		}
		...
		switch (defense)
		{
			...
			case 4:
				...
				break;
			case 5:
				Spy(1.5);//TODO: input time desired(use position?)
		}
	}
	...
	void RockWall(int pos)
	{
		...
		switch (rockWallState)
		{
			...
			case rockWallApproach:
				...
				break;
			case rockWallCross:
				if (vertGyro->GetAngle() >= 35)//keep us from tipping TODO: find angle(manually tilt)
				{
					setSpeed(0);
					//setSpeed(-.05);//if stopping isnt enough
				}
				else
				{
					setSpeed(.64);
	...
	void Spy(float time)
	{
		switch (spyState)
		{
			case spyIntake:
				...
				{
					setIntake(0);
					spyState = spyFirstTurn;
				}
				break;
			case spyFirstTurn:
				spyGyroPID(45, horzGyro->GetAngle(), .012, .002);
				if (absVal(spyGyroPIDError) < 5)
				{
					setSpeed(0);
					approachTimer->Reset();
					approachTimer->Start();
			...
			case spyDrive:
				setSpeed(.2);
				if (approachTimer->Get() > time)
				{
					setSpeed(0);
					spyState = spySecondTurn;
				}
				break;
			case spySecondTurn:
				spyGyroPID(0, horzGyro->GetAngle(), .012, .002);
				if (absVal(spyGyroPIDError) < 5)
				{
					setSpeed(0);
					spyState = spyShoot;
	...
	void spyGyroPID(float myTar, float myPos, float myP, float myI)
	{
		spyGyroPIDError = myTar - myPos;
		float PIDPout = PIDError * myP;
		float PIDIout = PIDError * myI;
		float spyGyroPIDSpeed = (PIDPout + PIDIout);

		left->Set(spyGyroPIDSpeed);
		right->Set(spyGyroPIDSpeed);
	...
	void autonShoot()
	{
		...
		switch (autonShootState)
		{
			case autonAligning:
				...
				if (turnRight && cogx == 217)
				{
					left->Set(.5);
					right->Set(.5);
				}
				else if (!turnRight && cogx == 217)
				{
					left->Set(-.5);
					right->Set(-.5);
				}
	...
	void SmartDash()
	{
		...
		string cogxString = "COG_X " + std::to_string(table->GetNumber("COG_X", 217));
		SmartDashboard::PutString("DB/String 0", cogxString);

		string cogyString = "COG_Y " + std::to_string(table->GetNumber("COG_Y", 217));
		SmartDashboard::PutString("DB/String 1", cogyString);

		string shooterString = "Shooter: " + std::to_string(shooter1->GetSpeed());
		...
		SmartDashboard::PutString("DB/String 4", PTOString);

		string gyroString = "VertGyro: " + std::to_string(vertGyro->GetRate());
