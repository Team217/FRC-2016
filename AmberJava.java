...
public class Robot extends IterativeRobot {
	...
	shootState currShootState = shootState.intake;
	...
	public void robotInit() {
		...
		// Relay
		ledSpike = new Relay(0, Relay.Direction.kForward);
	...
	/**
	 * All of the methods for crossing the moat in all positions
	 * @param pos the position the moat is in
	 */
	void Moat(int pos) {
		switch (pos) {
		case 1:
	...
	/**
	 * All of the methods for crossing the rock wall in all positions
	 * @param pos the position of the rock wall on the field
	 */
	...
	/**
	 * All of the methods for crossing the rough terrain in all positions
	 * @param pos the position of the rough terrain on the field
	 */
	...
	/**
	 * All of the methods for crossing the ramparts in all positions
	 * @param pos the position of the ramparts on the field
	 */
	...
	/**
	 * state machine for aligning and firing in autonomous
	 */
	void autonShoot() {
		cogx = table.getNumber("COG_X", 1000);
		cogy = table.getNumber("COG_Y", 1000);
	...
	void SmartDash() {
		...
		String cogxString = "COG_X " + String.valueOf(table.getNumber("COG_X", 1000));
		SmartDashboard.putString("DB/String 0", cogxString);

		String cogyString = "COG_Y " + String.valueOf(table.getNumber("COG_Y", 1000));
		...
		String HoodString = "Hood: " + String.valueOf(hood.getEncPosition());
		...
		String armString = "Arm: " + String.valueOf(arm.getEncPosition());
		...
		String gyroString = "VertGyro: " + String.valueOf(vertGyro.getAngle());
	...
	/**
	 * Selection of the defense and position for autonomous mode through the dashboard
	 */
	void AutonSelect() {
		String defString = SmartDashboard.getString("DB/String 5", "2");
		defenseSelected = Integer.parseInt(defString);  // what defense

		String posString = SmartDashboard.getString("DB/String 6", "2");
		positionSelected = Integer.parseInt(posString); // what position

		String autonFireString = SmartDashboard.getString("DB/String 7", "1");
		autonFireSelected = Integer.parseInt(autonFireString);

		String autonWaitString = SmartDashboard.getString("DB/String 8", "0");
		autonWaitSelected = Integer.parseInt(autonWaitString);
	}
	...
	void Hood() {
		...
		// stops hood from going past point
		if (hood.getEncPosition() < -1650) {
			hood.set(normPID(-1300, hood.getEncPosition(), .005, 0.0025));
		} else {
			...
			if (zeroHood.get() == false) {
				...
				hood.set(normPID(50, hood.getEncPosition(), .0057, 0.003));
	...
	// TODO: YES YOU GWELLY
	// TODO: IM NOT KIDDING
	// TODO: IF I COME TO PITS AND THIS ISNT DONE I S2GAO
	...
	double visionPID(double cogTar, double cogPos) {
		...
		if (cogPIDError > 40) {
			cogPIDSpeed = .35; // was .37
		} else if (cogPIDError > 5) {
			cogPIDSpeed = .27; // was .27
